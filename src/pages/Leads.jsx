...
export default function Leads() {
  ...
  // Blur names for Free plan
  const maskName = (name) => {
    if (plan === "pro" || userEmail === FREE_ACCESS_EMAIL) return name;
    if (!name) return "";
      ...
      <div className="px-5 sm:px-10 pt-14 pb-20 ml-0 md:ml-28 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-10">
          <div>
            ...
            <p className="text-gray-400 mt-2">Your AI assistant collects leads in real-time.</p>
          </div>
        ...
        <div className="min-h-[420px]">
          ...
          {activeTab === "timeline" &&
            (plan === "pro"
              ? <TimelineView leads={filteredLeads} maskName={maskName} />
              : <Locked text="Timeline is Pro only." />)
          }

          {activeTab === "universe" &&
            (plan === "pro"
              ? <UniverseGridView leads={filteredLeads} maskName={maskName} />
  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 px-5 py-2.5 rounded-xl transition-all duration-300
        ...
        ${active
          ? "border-purple-400/50 bg-purple-500/20 shadow-[0_0_20px_rgba(160,105,255,0.35)] text-purple-300"
...
function Locked({ text }) {
  return (
    <div className="flex flex-col items-center justify-center h-[380px]
                    ...
                    backdrop-blur-xl shadow-[0_0_25px_rgba(150,80,255,0.25)]">
      <Lock className="w-10 h-10 text-purple-300 mb-4 drop-shadow-[0_0_10px_rgba(180,120,255,0.6)]" />
...
function KPI({ title, value, icon, locked }) {
  return (
    <motion.div whileHover={{ scale: 1.03 }} className="rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur-xl">
...
function Sparkline({ trend7 }) {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-[140px] rounded-2xl p-4 mt-4 bg-white/5 border border-white/10 backdrop-blur-xl">
      {path && (
        <motion.path
          d={path}
          fill="none"
      ...
      {points.map(([x, y], i) => (
        <circle key={i} cx={x} cy={y} r="3" fill="#c4a2ff" />
      ))}
    </svg>
  );
}
...
/* 🕒 Timeline */
function TimelineView({ leads, maskName }) {
  ...
  return (
    <motion.div
      ...
      className="space-y-4 bg-white/5 border border-white/10 p-6 rounded-2xl backdrop-blur-xl shadow-[0_0_25px_rgba(120,60,255,0.25)]"
    >
      {leads.map((l) => (
        <div
          key={l.id}
          className="p-5 rounded-xl bg-white/10 border border-white/10 backdrop-blur-lg shadow-[0_0_15px_rgba(150,80,255,0.2)]"
        >
          <div className="font-semibold text-white text-lg">
            {maskName(l.name)}
          </div>

          <div className="text-purple-300 text-sm">{l.email}</div>
...
/* 🌌 Universe Grid */
function UniverseGridView({ leads, maskName }) {
  if (!leads?.length) return <Locked text="No leads yet." />;
      ...
      {leads.map((l) => (
        <motion.div
          ...
          className="p-6 rounded-2xl border border-white/10 bg-white/5 
                     backdrop-blur-xl shadow-[0_0_25px_rgba(160,80,255,0.25)]"
        >
          <h3 className="text-xl font-semibold text-white">{maskName(l.name)}</h3>
          <p className="text-purple-300 text-sm">{l.email}</p>
          <p className="text-gray-300 text-sm mt-2">{l.message}</p>
        </motion.div>
      ))}
    </motion.div>
